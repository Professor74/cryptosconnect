...
class CfCoinsSearchResult extends BxDolTwigSearchResult
{
    var $aCurrent = array(
        'name' => 'cf_coins',
        'title' => '_cf_coins_page_title_browse',
        'table' => 'cf_coins_main',
        ...
        'restriction' => array(
            ...
            'public' => array('value' => '', 'field' => 'allow_view_coin_to', 'operator' => '='),
        ),
    ...
    function CfCoinsSearchResult($sMode = '', $sValue = '', $sValue2 = '', $sValue3 = '')
    {
        switch ($sMode) {
            ...
            case 'search':
                ...
                $this->aCurrent['title'] = _t('_cf_coins_page_title_search_results') . ' ' . (is_array($sValue2) ? implode(', ',$sValue2) : $sValue2) . ' ' . $sValue;
                unset($this->aCurrent['rss']);
                break;

            case 'user':
                $iProfileId = $GLOBALS['oCfCoinsModule']->_oDb->getProfileIdByNickName ($sValue, false);
            ...
            case 'category':
                ...
                $this->aCurrent['title'] = _t('_cf_coins_page_title_browse_by_category', $sValue);
                break;
            ...
            case 'tag':
                ...
                $this->aCurrent['title'] = _t('_cf_coins_page_title_browse_by_tag', $sValue);
                break;

            case 'recent':
                $this->sBrowseUrl = 'browse/recent';
                $this->aCurrent['title'] = _t('_cf_coins_page_title_browse_recent');
                break;

            case 'top':
                $this->sBrowseUrl = 'browse/top';
                $this->aCurrent['sorting'] = 'top';
                $this->aCurrent['title'] = _t('_cf_coins_page_title_browse_top_rated');
                break;

            case 'popular':
                $this->sBrowseUrl = 'browse/popular';
                $this->aCurrent['sorting'] = 'popular';
                $this->aCurrent['title'] = _t('_cf_coins_page_title_browse_popular');
                break;

            case 'featured':
            $this->aCurrent['restriction']['featured'] = array('value' => 1, 'field' => 'featured', 'operator' => '=');
                $this->sBrowseUrl = 'browse/featured';
                $this->aCurrent['title'] = _t('_cf_coins_page_title_browse_featured');
                break;
            ...
            case 'calendar':
                ...
                $this->aCurrent['title'] = _t('_cf_coins_page_title_browse_by_day', getLocaleDate(strtotime("{$sValue}-{$sValue2}-{$sValue3}"), BX_DOL_LOCALE_DATE_SHORT));
                break;

            case '':
                $this->sBrowseUrl = 'browse/';
                $this->aCurrent['title'] = _t('_cf_coins');
                unset($this->aCurrent['rss']);
                break;
        ...
        $this->aCurrent['paginate']['perPage'] = $oMain->_oDb->getParam('cf_coins_perpage_browse');
    ...
    function getAlterOrder()
    {
        if ($this->aCurrent['sorting'] == 'last') {
            $aSql = array();
            $aSql['order'] = " ORDER BY `cf_coins_main`.`created` DESC";
            return $aSql;
        } elseif ($this->aCurrent['sorting'] == 'top') {
            $aSql = array();
            $aSql['order'] = " ORDER BY `cf_coins_main`.`rate` DESC, `cf_coins_main`.`rate_count` DESC";
            return $aSql;
        } elseif ($this->aCurrent['sorting'] == 'popular') {
            $aSql = array();
            $aSql['order'] = " ORDER BY `cf_coins_main`.`fans_count` DESC, `cf_coins_main`.`views` DESC";
            return $aSql;
        }
        return array();
    }
